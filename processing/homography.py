        ...
        if None not in inters:
            ...
            for i in range(12):
                if i not in inds and points[i] is not None:
                    # Flatten points[i] and trans_kps[i] if not already 1D
                    point = np.array(points[i]).flatten()
                    trans_kp = np.array(trans_kps[i]).flatten()
                    dists.append(distance.euclidean(point, trans_kp))
